...
myapp.controller("signupCtrl", [
  ...
  function ($scope, $location, AuthService, $http) {
    $http
      .get(BaseUrl + "/api/v1/ngos")
      .success(function (data, status, header, config) {
        console.log(data);
        var options = "<option>Select your NGO</option>";
        $.each(data.NGOs, function (key, value) {
          options +=
            "<option value='" + value.id + "'>" + value.name + "</option>";
        });
        $("#organization_list").html(options);
      });
    $scope.message = false;
    $scope.sign_up = function (fullname, email, password, org_name) {
      $scope.error = false;
      $scope.disable = true;
      var status = false;
      // call register service
      var ngo_id_selected = $("#organization_list").val();
      console.log(ngo_id_selected, "should I register with this id====>");
      AuthService.registerUser(fullname, email, password, ngo_id_selected)
        .then(function () {
  ...
  function ($scope, $http, $location, AuthService) {
    ...
    $scope.sign_in = function (username, password) {
      ...
      AuthService.login(data)
        .then(function () {
          $location.path("/validate-otp-code");
        })
        .catch(function () {
          $scope.message = true;
        });
...
myapp.controller("validateOtpCtrl", [
  "$scope",
  "$http",
  "$location",
  "$routeParams",
  ...
  function ($scope, $http, $location, $routeParams, AuthService) {
    $scope.notValid = true;
    $scope.validateOtpCode = function (otp) {
      var data = '{"code":"' + otp + '"}';
  ...
  function ($scope, $location, AuthService) {
    ...
    $scope.recoverPass = function (email) {
      ...
      AuthService.recover(email)
        .then(function () {
          $scope.valid = true;
          $scope.reset_email = false;
          $scope.reset_email_success = true;
        })
        .catch(function () {
          $scope.notValid = true;
          $scope.reset_email = true;
...
myapp.controller("excelFileCtrl", [
  "$scope",
  "Upload",
  "$timeout",
  "$window",
  ...
  "$location",
  "AuthService",
  function ($scope, Upload, $timeout, $window, $http, $location, AuthService) {
    $("body").removeClass("body-login");
    $("body").addClass("body-app");
    ...
    function savepdf(original, filename) {
      ...
      $http
        .post(BaseUrl + "/api/v1/file/save/", data, config)
        .success(function (data, status, header, config) {
          console.log(data);
    ...
    function multipleDataView(json_data) {
      ...
      $.each(json_data, function (key, value) {
        ...
        male.push(parseInt(value["sgs_members__male"]));
    ...
    $scope.saveData = function (originalpath, filename) {
      ...
      $http
        .post(BaseUrl + "/api/v1/file/user/", data, config)
        .success(function (data, status, header, config) {
          if (data.auth) {
            console.log(data);
            $scope.showDataUlploaded = false;
  ...
  function ($scope, $http, $location) {
    ...
    $(window).resize(function () {
      ...
      if ($(window).width() < 1080) {
        $("main").hide();
        $("body").html(
          "<h2 class='text-center'>Oops!</h2><h4 class='text-center'>Switch to bigger screen greater than 1024px to view the map</h4>"
        );
      } else {
        location.reload();
      }
    ...
    $("input[type=radio][name=upload]").change(function () {
      var val = this.value;
      var url = BaseUrl + "/api/v1/params/upload/" + user_id + "/" + val;
      console.log(user_id, "=========");
      $http
        .put(url)
        .success(function (data, status, header, config) {
    ...
    $("input[type=radio][name=signup]").change(function () {
      var val = this.value;
      var url = BaseUrl + "/api/v1/params/signup/" + user_id + "/" + val;
      console.log(user_id, "=========");
      $http
        .put(url)
        .success(function (data, status, header, config) {
    ...
    $("#change_password").click(function (e) {
      ...
      var url = BaseUrl + "/api/v1/users/check_password/" + user_id;
    ...
    function check_new_password(new_password, confirm_password) {
      if (new_password == confirm_password) {
        ...
        var url = BaseUrl + "/api/v1/change/password/" + user_id;
        $http
          .put(url, data, config)
          .success(function (data, status, header, config) {
            if (data) {
    ...
    function sendEmailMessage(title, message, email, name, username) {
      var data =
        ...
        username +
        '"}';
      var url = BaseUrl + "/api/v1/help/message/";
      $http
    ...
    function loadSavingYear() {
      $http
        .get(BaseUrl + "/api/v1/saving_year/")
        .success(function (data, status, header, config) {
          console.log(data);
          var options = "";
...
myapp.controller("notificationCtrl", [
  ...
  "AuthService",
  "$q",
  function ($scope, $http, AuthService, $q) {
    var user_id = localStorage.getItem("u___");
    var url = BaseUrl + "/api/v1/user/" + user_id;
      ...
      function loadNgoFiles(user_id) {
        ...
        $http
          ...
          .success(function (data, status, header, config) {
            console.log(
              data,
              "am heree should print the NGO data and display them"
            );
            $scope.files = data;
          })
          .error(function (data, status, header, config) {});
...
myapp.controller("viewAlldataCtrl", [
  "$scope",
  "$http",
  "AuthService",
  "$q",
      ...
      $http
        ...
        .success(function (data, status, header, config) {
          if (window.navigator.msSaveOrOpenBlob) {
            window.navigator.msSaveBlob(blob, "saving_groups.xls");
          } else {
            ...
            downloadLink.download = "saving_groups.xls";
...
function loadProvinceSelectBox(idBox, $http) {
  $http
    .get(BaseUrl + "/api/v1/kenessa/province/province/all")
    .success(function (data, status, header, config) {
      console.log(data);
      var options = "";
      $.each(data, function (key, value) {
...
function loadDistrictSelectBox(province_id, idBox, $http) {
  $http
    .get(BaseUrl + "/api/v1/kenessa/province/district/" + province_id)
    .success(function (data, status, header, config) {
      console.log(data);
      var options = "";
      $.each(data, function (key, value) {
...
function chartFunction($http, year, ngo, province, district) {
  ...
  $http
    .get(
      BaseUrl +
        "/api/v1/chartanalytics/membership/" +
        year +
        "/" +
        ngo +
        "/" +
  ...
  $http
    .get(
      BaseUrl +
        "/api/v1/chartanalytics/financial/" +
        year +
        "/" +
        ngo +
        "/" +
  ...
  $http
    ...
    .success(function (data, status, header, config) {
      ...
      Plotly.newPlot(
        "finscope_sg_pie_2012",
        [data.finscope_sg_2012[0][0]],
        layout_sg
      );
      ...
      Plotly.newPlot(
        "finscope_sg_pie_2015",
        [data.finscope_sg_2015[0][0]],
        layout_sg
      );
      // Finscope all 2012
...
function leafletCartix(year) {
  ...
  var Jsonfile;
  // http://209.97.190.128:5000/api/v1/sqlsaving
  // assets/geojson/stats.json

  /*function jsonData(handleData){
        $.getJSON(BaseUrl+'/api/v1/sqlsaving', function(data) {
            //console.log(data);
            handleData(data);
        }); 
    }
  ...
  url = BaseUrl + "/api/v1/sqlsaving/" + sg_ngo + "/" + year;
  $("#saving_group_map").change(function () {
    sg_ngo = $("#saving_group_map").val();
    year = $("#year").val();
    url = BaseUrl + "/api/v1/sqlsaving/" + sg_ngo + "/" + year;
  ...
  info.update = function (
    ...
    this._div.innerHTML =
      "<h4>Saving groups per Province</h4>" +
      (props
        ? "<b>" +
          props.Name +
          " Province</b><br />" +
          density +
          " Saving groups<br/> <b> Total Membership: </b>" +
          membr +
          "<br/> <b> Total Female: </b>" +
          fem +
          " <br/> <b>Total Male: </b>" +
          male +
          "</br><b>Total Out. Loans: </b>" +
          borrowing +
          "<br><b>Total Savings: </b>" +
          saving +
          "<br><b> Banks: </b>" +
          banks +
          "</br><b>MFIs: </b>" +
          mfi +
          "</br><b> Non-Umurenge Sacco: </b>" +
          nu +
          "</br><b>Umurenge Sacco:</b>" +
          u +
          "</br><b>Telco Agents: </b>" +
          tl +
          "</br><b>Bank Agents:</b>" +
  ...
  infoP.update = function (
    ...
    this._div.innerHTML =
      "<h4>Saving groups in each District per Province</h4>" +
      (props
        ? "<b>" +
          props.Name +
          " District</b><br />" +
  ...
  infoD.update = function (
    ...
    this._div.innerHTML =
      "<h4>Saving groups per District</h4>" +
      (props
        ? "<b>" +
          props.Name +
          " District</b><br />" +
  ...
  infoS.update = function (
    ...
    this._div.innerHTML =
      "<h4>Saving groups per Sector</h4>" +
      (props
        ? "<b>" +
          props.Name +
          " Sector</b><br />" +
