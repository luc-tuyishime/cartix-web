myapp.factory('AuthService', ['$q', '$timeout', '$http', function($q, $timeout, $http) {
    ...
    function login(data) {
        ...
        $http.post('https://api.cartix.io/api/v1/login/', data, config)
            // handle success
    ...
    function registerNgo(org_name, org_type) {
        ...
        $http.post('https://api.cartix.io/api/v1/ngo/', data, config)
            .success(function(data, status) {
                if (status == 200 && data.result) {
                    saveNgo(data.result);
                    deferred.resolve();
    ...
    function Key(email, key){
        ...
        // send a put request
        var link = 'https://api.cartix.io/api/v1/check/key/'+email+'/'+key;
        $http.get(link)
            .success(function(data,status){
                if(status == 200 && data.result){
                  deferred.resolve();  
    ...
    function changePassword(password, email){
        ...
        //send post request 
        $http.put('https://api.cartix.io/api/v1/change/password', data, config)
            .success(function(data, status){
                if(status == 200 && data.result){
                    console.log(data.result);
                    deferred.resolve();
    ...
    function recover(email){
        var deferred = $q.defer();
        var link = 'https://api.cartix.io/api/v1/recover/'+email;
    ...
    function registerUser(fullname, email, password, ngo_id) {
        ...
        $http.post('https://api.cartix.io/api/v1/user/', data, config)
            .success(function(data, status) {
                if (status == 200 && data.result) {
                    deferred.resolve();
                } else {
